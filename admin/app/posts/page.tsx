...
export default function PostsPage() {
  ...
  const loadPosts = async () => {
    setLoading(true);
    const data = await fetchAdminApi("/posts");
    if (data && data.data) {
      setPosts(data.data);
    }
    setLoading(false);
    ...
    <div className="flex flex-col w-full max-w-7xl mx-auto px-0 sm:px-4 lg:px-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div className="flex flex-col gap-1">
          <h1 className="text-slate-900 text-3xl font-extrabold leading-tight tracking-tight">Quản Lý Bài Viết Chung</h1>
        ...
        <Link href="/posts/create" className="flex items-center justify-center gap-2 rounded-lg h-11 px-6 bg-primary text-white text-sm font-bold shadow-lg shadow-primary/25 hover:bg-primary-dark transition-all w-full sm:w-auto text-center">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center text-slate-400">Chưa có bài viết nào.</td>
                  </tr>
                ) : posts.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-slate-400 truncate">slug: {item.slug}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 py-4 sm:py-5 text-right">
                      <div className="flex justify-end gap-1 sm:gap-2">
                        <Link href={`/posts/update/${item.id}`} className="p-1 sm:p-1.5 text-slate-400 hover:text-primary transition-colors bg-white rounded-md shadow-sm border border-slate-200 block">

  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setForm({
        ...
        is_overview_page: data.is_overview_page === 1 || data.is_overview_page === true,
        status: data.status || 'active',
        display_order: data.display_order || 0,
      });

      // Fetch projects
      const projectsRes = await fetch(`${API_BASE}/projects?per_page=1000`, {
        headers: { 'Accept': 'application/json' }
      });
      if (projectsRes.ok) {
        const projectsData = await projectsRes.json();
        const items = (projectsData.data || projectsData) as any[];
        setProjects(items.map(item => {
          const pTrans = item.translations?.find((t: any) => t.locale === 'vi') || item.translations?.[0];
          return {
            id: item.id,
            name: item.name || pTrans?.name || `Project ${item.id}`
          };
        }));
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col w-full max-w-4xl mx-auto px-4 lg:px-6">
      <div className="flex items-center gap-3 mb-6 sm:mb-8">
        ...
        <div>
          <h1 className="text-slate-900 text-2xl sm:text-3xl font-extrabold tracking-tight">Cập Nhật Phân Khu</h1>
          <p className="text-slate-500 text-sm">Chỉnh sửa thông tin phân khu "{form.name}".</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="space-y-6">
            ...
            {field('Tên Phân Khu *', 'name', 'Vd: The Landmark', 'text', undefined, (val) => {
              if (!form.slug) setForm(prev => ({ ...prev, slug: slugify(val) }));
            })}

...
export default function UpdateZoneArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const initData = async () => {
      setFetching(true);
      const [articleData, zoneData] = await Promise.all([
        fetchAdminApi(`/zone-articles/${id}`),
        fetchAdminApi("/zones")
      ]);

      if (zoneData) setZones(zoneData.data || zoneData);
      
      if (articleData) {
        const translation = articleData.translations[0] || {};
        setFormData({
          zone_id: articleData.zone_id?.toString() || "",
          type: articleData.type || "overview",
          layout_type: articleData.layout_type || "default",

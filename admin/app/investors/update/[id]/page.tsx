...
export default function UpdateInvestorPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error('Không thể tải dữ liệu chủ đầu tư');
      const data = await res.json();
      
      const viTrans = data.translations?.find((t: any) => t.locale === 'vi') ?? data.translations?.[0];
      
      setForm({
        name: data.name ?? viTrans?.name ?? '',
        ...
        short_description: data.short_description ?? viTrans?.short_description ?? '',
        content: data.content ?? viTrans?.content ?? '',
        stats: viTrans?.stats ?? [],
        benefits: viTrans?.benefits ?? [],
  ...
  return (
    <div className="flex flex-col w-full max-w-5xl mx-auto px-4 lg:px-6">
      <div className="flex items-center gap-3 mb-6 sm:mb-8">
        ...
        <div>
          <h1 className="text-slate-900 text-2xl sm:text-3xl font-extrabold tracking-tight">Cập Nhật Chủ Đầu Tư</h1>
          <p className="text-slate-500 text-sm">Chỉnh sửa hồ sơ chủ đầu tư đối tác.</p>
        </div>
      </div>

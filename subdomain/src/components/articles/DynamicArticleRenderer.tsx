...
export default function DynamicArticleRenderer({ article }: { article: ProjectArticle }) {
  ...
  switch (layout) {
      ...
      return (
        <section className="pt-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <LinkWrapper className="space-y-8">
              {/* 1. Hình ảnh phía trên (Width full theo text, không bị cắt xén) */}
              {article.banner_image_url && (
                <div className="w-full">
                  <img src={article.banner_image_url} className="w-full h-auto object-contain" alt={title} />
                </div>
              )}

              {/* 2. Nội dung chi tiết phía dưới */}
              {html_content && (
                <div className="prose prose-slate max-w-none text-gray-700 leading-relaxed text-[16px]" dangerouslySetInnerHTML={{ __html: html_content }} />
              )}

              {/* 3. Lấy thông tin Tóm tắt để hiện Google Map hoặc Text */}
              {summary && (
                <div className="w-full mt-8">
                  {summary.includes("<iframe") ? (
                    <div dangerouslySetInnerHTML={{ __html: summary }} className="w-full h-full [&>iframe]:w-full [&>iframe]:h-[500px] [&>iframe]:min-h-[500px] [&>iframe]:border-none block" />
                  ) : summary.startsWith("http") && summary.includes("google") ? (
                    <iframe src={summary} className="w-full h-[500px] border-none" allowFullScreen loading="lazy"></iframe>
                  ) : (
                    <div className="text-gray-700 text-lg leading-relaxed">{summary}</div>
                  )}
                </div>
              )}
    ...
    case "news_list":
      return (
        <section className="pt-20 bg-gray-50 border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4">
      ...
      return (
        <section className="py-14 bg-[#1a4b8c] text-white w-full overflow-hidden">
          <div className="max-w-7xl mx-auto px-6 text-center">
            ...
            {targetLink && (
              <div className="mt-12">
                <Link href={targetLink} className="inline-block px-12 py-4 border-2 border-white text-white font-bold rounded-full hover:bg-white hover:text-[#1a4b8c] transition-all duration-300 uppercase tracking-widest text-sm">
                  Khám Phá Chi Tiết
                </Link>
              </div>
            )}
